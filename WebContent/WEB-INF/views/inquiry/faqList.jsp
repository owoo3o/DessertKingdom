      ...
      <div class="faq">
        ...
        <div class="board_list_wrap">
            <table class="board_list" id="listArea">
              ...
              <!-- button area -->
               <!-- 로그인 멤버가  매니저 (MANAGER="Y" 일때만 작성하기 버튼 보이게하기 -->
               
               	<% if(loginMember != null && loginMember.getManager().equals("Y")) { %> 
               <div class="btn-box">
                <input type="button" onclick="location.href='<%= request.getContextPath() %>/writeFaqForm.bo'" value="작성하기">
               </div> 
           		<% } %> 
            ...
            <div class="paging">
              <!-- 맨 처음으로 -->
              <button class="btnOn" onclick="location.href='<%= request.getContextPath() %>/list.faq?currentPage=1'">&lt;&lt;</button>
              <!-- 이전 페이지로  -->
              <button class="btnOn" id="beforeBtn" onclick="location.href='<%= request.getContextPath() %>/list.faq?currentPage=<%= pi.getCurrentPage()-1 %>'">&lt;</button>
              <!-- 숫자 페이지로-->
              <% for(int p = pi.getStartPage(); p <= pi.getEndPage(); p++){ %>
              	<% if(p == pi.getCurrentPage()) { %>
              		<button id="choosen" disabled><%= p %></button>
              	<% } else { %>	
              		<button class="btnOn" onclick="location.href='<%= request.getContextPath() %>/list.faq?currentPage=<%= p %>'"><%= p %></button>
             	<% } %>
           	<% } %>
              <!-- 다음 페이지로  -->
              <button class="btnOn" onclick="location.href='<%= request.getContextPath() %>/list.faq?currentPage=<%= pi.getCurrentPage()+1 %>'" id="afterBtn">&gt;</button>
              <!-- 맨 마지막으로 -->
              <button class="btnOn" onclick="location.href='<%= request.getContextPath() %>/list.faq?currentPage=<%= pi.getMaxPage() %>'">&gt;&gt;</button>
            </div> 
          </div> 
        </div>
    </div>
 	...
 	// 페이징 버튼 스크립트
  	...
  	$(function(){
  		$('#listArea td').mouseenter(function(){
  			...
  			// 회원, 비회원 모두 열람 가능
  				location.href = '<%= request.getContextPath() %>/detail.faq?bId=' + bId;
  		});
  	});
