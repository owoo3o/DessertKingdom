<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="inquiry.model.vo.Faq, member.model.vo.Member" %>
<%
          ...
          <div class="detail_outer">
            <br>
              <div class="table-area">
                <form action="<%= request.getContextPath() %>/faqUpdateForm.faq" method="post" id="detailForm">
                      <!-- 글 번호 숨기기 -->
                      <input type="hidden" name="bId" value="<%= faq.getBoardId() %>">
                  <table id="faq-area">
                    <tr>
                      <td class="tr_title">제목</td>
                      <td class="td_content" colspan="5">
                      <%= faq.getBoardTitle() %>
                        <input type="hidden" id="faqTitle" name="faqTitle" value="<%= faq.getBoardTitle() %>">
                      </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      <td class="td_content">
                      <%= faq.getCateName() %>
                        <input type="hidden" id="category" name="category" value="<%= faq.getCateName() %>" >
                      </td>
                    </tr>
                    <tr>
                      <td colspan="6"><textarea name="content" cols="70px" rows="15px" style="resize:none;" readonly><%= faq.getBaordContent() %></textarea></td>
                    </tr>
                  </table>

                    <!-- button area -->
                    <div class="btn-box">
                    	<% if(loginMember != null && faq.getBoardWriter().equals(loginMember.getUserId())) {%>
	                        <input type="submit" id="updateBtn" value="수정">
	                        <input type="button" onclick="deleteBoard();" id="deleteBtn" value="삭제">
                        <% } %>
                        <input type="button" onclick="location.href='<%= request.getContextPath() %>/list.faq'" id="menuBtn" value="목록">
                    </div>
  	...
  	<script>
  		function deleteBoard() {
  			if(confirm('정말 삭제하시겠습니까?')) {
  				location.href="<%= request.getContextPath() %>/delete.faq?bId=" + <%= faq.getBoardId() %>
  			}
  		}
